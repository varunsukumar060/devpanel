...
def set_governor_direct(gov: str) -> tuple[bool, str]:
    """
    Write CPU governor directly to sysfs without sudo.
    Works if:
      (a) devpanel is run as root, OR
      (b) user has a sudoers rule for cpufreq, OR
      (c) cpufreq files are group-writable (some distros).
    ...
    if success_count == cpu_count:
        return True, f"✔ Governor set to [bold]{gov}[/] on all {cpu_count} CPUs"
    elif success_count > 0:
        return True, f"✔ Governor set to [bold]{gov}[/] on {success_count}/{cpu_count} CPUs"
    else:
        if last_err == "permission":
            return False, (
                f"[yellow]⚠ Permission denied.[/]\n"
                f"  Run one of these to fix:\n"
                f"  [cyan]Option 1:[/] Relaunch with sudo:  [bold]sudo bash run.sh[/]\n"
                f"  [cyan]Option 2:[/] Add a sudoers rule (no password needed):\n"
                f"    [bold]sudo visudo -f /etc/sudoers.d/devpanel-cpufreq[/]\n"
                f"    Add line:  [bold]{os.environ.get('USER','$USER')} ALL=(ALL) NOPASSWD: /usr/bin/tee /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor[/]"
            )
        elif last_err == "not_found":
            return False, "[yellow]⚠ cpufreq not available on this CPU (no scaling_governor file found)[/]"
        else:
            return False, f"[red]✖ Error: {last_err}[/]"
...
def get_top_procs(n=10) -> list[dict]:
    procs = []
...
class HUDTab(Static):
    ...
    def refresh_hud(self) -> None:
        ...
        temps = get_thermal()
        cpu_temp = next((s["current"] for s in temps if any(k in s["label"].lower() for k in ["cpu","core","package","tdie","tctl"])), None)
        ...
        running_as = "[green]root (full access)[/]" if os.geteuid() == 0 else "[yellow]user (cpufreq may need sudo)[/]"

        text = f"""[bold cyan]╔══ SYSTEM HUD ═════════════════════════════════════╗[/]
[bold cyan]║[/] {datetime.now().strftime('%H:%M:%S')}  Uptime: {uptime}  Running as: {running_as}
[bold cyan]╚════════════════════════════════════════════════╝[/]
...
class ThermalTab(Static):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Static(
            "  [dim]Tip: If buttons show permission error, relaunch with:[/]\n"
            "  [cyan]sudo bash run.sh[/]  (enables full cpufreq control)"
        )
    ...
    def refresh_thermal(self) -> None:
        ...
        if sensors:
            ...
            for s in sensors:
                ...
                lines.append(f"  {s['label']:<22} [{col}]{t:5.1f}°C[/]  {b}  (crit: {s['critical'] or '?'}°C)")
        else:
        ...
        avail = run("cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_available_governors 2>/dev/null") or "unknown"

        lines.append(f"\n[bold]── CPU Frequency ─────────────────────────────[/]")
        lines.append(f"  Governor  : [cyan]{gov}[/]")
        lines.append(f"  Cur Freq  : [cyan]{freq_mhz}[/]")
        lines.append(f"  Available : [dim]{avail}[/]")
    ...
    def _set_governor(self, gov: str) -> None:
        # Direct sysfs write — no sudo prompt, no terminal conflict
        status_widget = self.query_one("#thermal-status", Static)
        status_widget.update(f"  [dim]Setting governor to {gov}...[/]")
...
class BootTab(Static):
    ...
    def refresh_boot(self) -> None:
        ...
        lines.append("[bold]── Boot Summary ──────────────────────────────[/]")
        ...
        if services:
            ...
            for s in services[:15]:
                try:
                    raw = s["time"].replace("min","min").replace("ms","").replace("s","000").split(".")[0]
                    ms_val = int(''.join(filter(str.isdigit, s["time"].split(".")[0])) or 0)
                    col = "red" if "min" in s["time"] else ("yellow" if ms_val > 2000 else "green")
                except Exception:
                    col = "white"
...
class DevPanel(App):
    ...
    def action_refresh_all(self) -> None:
        for cls in [HUDTab, ReposTab, ThermalTab, MemoryTab, BootTab, WorkspaceTab]:
            try:
                widget = self.query_one(cls)
                for method in ["refresh_hud","refresh_repos","refresh_thermal","refresh_mem","refresh_boot","refresh_ws"]:
